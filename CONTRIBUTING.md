...
The catalog derives `owner` and the plugin's source location from `id`, which prevents duplicated fields from drifting: a two-segment id names the repository itself, and a subdirectory id pins the plugin's manifest at `<sub/dir>/package.json`. The install command comes from the plugin's published npm package, never from the id. Id path segments may not be `.` or `..`.

Every pull request receives one deliberately narrow static gate. The workflow rejects any change outside `catalog/plugins/*.json`. For every added or modified entry it validates the exact fields, filename, category, descriptions, and date, then reads the target repository through the GitHub API. For a two-segment id it finds a root or nested `package.json` with `dsh.bundle.patch`; for a subdirectory id it requires that manifest exactly at the id's path. In both cases it confirms that the patch path exists in the same revision. It also reminds the author on the pull request that store installs come from npm only — a plugin without a published npm package is listed browse-only — but never rejects a submission over it: installability is a published label, not an admission test. Deleted entries carry no content to validate. The gate does not install dependencies, run lifecycle scripts, parse the patch, build the project, or assess plugin behavior.
