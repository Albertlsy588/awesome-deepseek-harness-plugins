...
export function compareVersions(leftValue: string, rightValue: string): number {
  ...
  if (left === null || right === null) throw new Error('update API returned an invalid semantic version')
...
async function fetchManifest(url: string, fetcher: typeof fetch): Promise<PackageManifest> {
  const endpoint = new URL(url)
  if (endpoint.protocol !== 'https:') throw new Error('update API URL must use HTTPS')
...
export async function checkForUpdate(
  ...
  fallbackUrl: string = DEFAULT_UPDATE_FALLBACK_URL,
  fetcher: typeof fetch = fetch,
  lastResortUrl: string = DEFAULT_UPDATE_LAST_RESORT_URL,
): Promise<UpdateInfo> {
  const errors: string[] = []
