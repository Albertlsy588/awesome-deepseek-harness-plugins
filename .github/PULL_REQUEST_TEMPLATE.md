## Plugin

Link the submitted plugin repository and summarize its user-visible capability.

## Author verification

List the real command and result used to test plugin behavior and compatibility. The catalog workflow does not execute third-party plugin code.

## Catalog submissions

- [ ] This PR adds exactly one `catalog/plugins/*.json` file and changes nothing else
- [ ] Repository declares `dsh.bundle.patch` and commits the referenced patch file
- [ ] Plugin behavior and compatibility were tested by the author
- [ ] `dsh-plugin` topic added
- [ ] English and Chinese descriptions are neutral and accurate
- [ ] I understand that a failed review leaves this PR open for corrections; a non-draft PR passing the review is merged automatically, and the website catalog and README directories refresh automatically afterwards
