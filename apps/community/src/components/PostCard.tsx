...
export function PostCard({ post, variant = 'feed', onChange, onRemoved }: PostCardProps) {
  ...
  async function copyLink() {
    const url = `${window.location.origin}/p/${post.replyToId ?? post.id}`
    try {
  ...
  return (
    <article className={`post post-${variant}${deleted ? ' is-deleted' : ''}`}>
      <Link className="post-avatar" to={`/u/${post.author.login}`} aria-label={post.author.login}>
        <Avatar login={post.author.login} src={post.author.avatarUrl} size={variant === 'reply' ? 32 : 40} />
      </Link>

      <div className="post-main">
        <header className="post-head">
          <Link className="post-author" to={`/u/${post.author.login}`}>
          ...
          {variant === 'feed'
            ? <Link className="post-time" to={`/p/${post.id}`}>{timestamp}</Link>
        ...
        {deleted ? null : (
          <footer className="post-actions">
            ...
            {post.replyToId === null ? (
              <Link className="post-action" to={`/p/${post.id}`} aria-label={t('reply')}>
