...
- 不得手动合并 PR，也不得修改生成投影。合规的非草稿目录 PR 会在可信静态审查通过后自动合并；插件被接受后，由维护者单独更新生成文件。
